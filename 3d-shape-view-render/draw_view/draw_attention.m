...
for fi = 1:20
    ...
    [vs, fs] = get_attention_pos(r_vec,v(fi,:) + 0.015 * r_vec,50,5000 * attention(fi));
    patch('Faces',fs,'Vertices',vs,'FaceColor',colors(fi,:));
    surf(xI,yI,zI,'CData',img,'FaceColor','texturemap');
%     text(double(v(fi,1)),double(v(fi,2)),double(v(fi,3)),num2str(fi));
end

% % load attention images
% for fi = 1:20
% %     img = imread(sprintf('./data/airplane_0627_%03d.jpg',fi*4 - 3));
%     img = imread(sprintf('./attention/%03d.jpg',fi));
%     img = fliplr(img);
%     r_vec = v(fi,:)-center;
%     [xI,yI,zI] = get_mesh_pos(r_vec,v(fi,:) + 0.5 * r_vec,200);
%     [vs, fs] = get_attention_pos(r_vec,v(fi,:) + 0.5 * r_vec,50,100);
%     patch('Faces',fs,'Vertices',vs,'FaceColor','b');
%     surf(xI,yI,zI,'CData',img,'FaceColor','texturemap');
% %     text(double(v(fi,1)),double(v(fi,2)),double(v(fi,3)),num2str(fi));
% end
